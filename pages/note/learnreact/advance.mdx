...
但是这里我们不只要讲这种方式，我们还要讲一种更好的方式，那就是使用 Tailwind CSS。

### Tailwind CSS

这是 Next.js 官方推荐的一种样式解决方案，它是一个功能强大的工具集，可以帮助我们快速构建自定义的用户界面。无需重新安装，无需编写 CSS，只需使用现有的 HTML 和 CSS 类，就可以快速构建出一个漂亮的界面。

例如上面的例子，我们可以使用 Tailwind CSS 来实现同样的效果。

删除 `index.module.css` 文件，然后在 `/pages/about/index.js` 中将原内容替换为下面的内容：
...
<div className={'my-3'}>
  <SandpackProvider
    ...
    files={{
      ...
      '/components/footer.js': { code: Footer },
      '/components/logo.js': { code: Logo },
      '/pages/about.js': { code: NewAboutWithTailwind, active: true }
    }}
  >
    <SandpackLayout>
